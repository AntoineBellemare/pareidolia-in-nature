...
empirical scaffolding for exactly this coupling view, on a
philosophical footing Merleau-Ponty had already laid: perceiving is
not a spectator's representation of a world set apart, but the
engagement of a body always already situated in
it~\citep{merleauponty1945phenomenology}. Gibson's
...
this beyond perception, drawing Merleau-Ponty's phenomenology into
cognitive science
explicitly~\citep{varela1991embodied,clark1998extended,noe2004action}.
...
% =============================================================================
% Bibliography. To keep Overleaf's free-plan compile inside the timeout we
% ship a precompiled bibliography (paper.bbl) and \input it directly, so
% BibTeX never runs: the build is 2 fast LaTeX passes instead of 4. If
% paper.bbl is absent the guard falls back to the normal \bibliography +
% BibTeX path, so nothing breaks.
% TO REGENERATE after editing references.bib: delete paper.bbl, then
% compile once (locally: tectonic paper.tex). This reruns BibTeX from
% references.bib + \bibliographystyle{plainnat} and rewrites paper.bbl.
\IfFileExists{paper.bbl}{\input{paper.bbl}}{\bibliography{references}}
