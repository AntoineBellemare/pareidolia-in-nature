...
\usepackage{natbib}
% EHS asks for APA. natbib+plainnat already yields APA's in-text form --
% (Smith, 2012), (Smith and Wright, 2013), (Smith et al.) -- and sorts the
% reference list alphabetically by first author surname. Reference-list
% micro-formatting is explicitly "not grounds for rejection at the
% submitting phase"; finish the APA conversion at revision.
\bibliographystyle{plainnat}
% EHS: "Please include continuous line numbers."
% Pin the line-number font to the main (Times) face: lineno's default is
% \normalfont\tiny\sffamily, which pulls in a font xdvipdfmx cannot
% resolve to a physical font under this fontspec setup.
% EHS: "Please include continuous line numbers."
\usepackage{lineno}
\usepackage{titlesec}
...
% EHS: title "should be short (preferably up to 12 words)". This is 12.
\title{\bfseries The Imagination of Nature:\\
\large Mythology as a Recording Layer of Perceptual Coupling}

\author[1,*]{Antoine Bellemare-Pepin}
\author[2]{Mar Estarellas}
\affil[1]{Department of Psychology, Université de Montréal, Montréal, Canada}
\affil[2]{Department of Psychiatry, McGill University, Montréal, Canada}
\affil[*]{Corresponding author. Email: \texttt{antoine.bellemare9@gmail.com}}
\date{\today}
...
% EHS title page requires a word count "including all text but excluding
% tables, figures and references".
\begin{center}
\small\textbf{Word count:} 7{,}9XX (main text; excludes abstract, tables,
...
alone? Recent theories of culture recast this as a question of how
heavily ecology weights the forms a tradition settles into. We measure
that weight across Berezkin's folk-motifs from 958 traditions, each
geocoded to one of the 14 WWF biomes, embedding each motif's text with
sentence-pooled SigLIP-2 and aligning it to imagery of that biome
(iNaturalist photographs and Places365 scenes). A
tradition's mythology aligns with its own biome's imagery, and the
alignment survives the names a myth uses: it holds whether that
vocabulary is removed before embedding, held constant under a
matched-permutation null, or read off the unsupervised myth-image
geometry without biome labels, three convergent strategies that agree
on the same biomes. The alignment decays monotonically with motif
breadth, strongest in biome-specific motifs and fading toward zero in
universal ones; this gradient is the load-bearing signature. What a
mythology carries of its biome lives in the schemas of attention,
activity, and form a place makes available to imagination. Mythology
records that coupling, and its ecological weight is now measurable at
scale.
\end{abstract}

\vspace{0.4em}

% EHS: "you will be asked to include a minimum of three keywords."
% NB: keep \small inside the group -- an unbraced \small here leaks into the
% rest of the document and drags nested math down to 5pt scriptscript,
% which needs cmmi5 and breaks the PDF stage.
{\noindent\small\textbf{Keywords:} cultural evolution; cultural attraction;
human ecology; folklore; vision--language models\par}
...
perceiver and an articulate world. We call this second view the
...
These threads converge, in more recent work, on a picture that
supersedes the projection-versus-coupling choice rather than taking
a side in it. Cultural attraction theory treats the stability of a
cultural form not as a victory of mind over matter or matter over
mind, but as its settling at an \emph{attractor} fixed jointly by
cognitive factors of attraction --- what minds find memorable,
inferable, and transmissible --- and \emph{ecological} factors of
attraction, the material and environmental conditions under which the
form is repeatedly produced and
reproduced~\citep{sperber1996explaining,claidiere2014darwinian,morin2016traditions}.
Cognitive ecology names the same web of mutual dependence among brain,
body, environment, and culture~\citep{hutchins2010cognitive}; the
cultural-affordances programme casts a lived environment as a
structured landscape that scaffolds attention and shared
sense-making~\citep{ramstead2016cultural}; and niche-construction
accounts formalise the loop by which the worlds people build come to
shape the cognition of those who inherit
them~\citep{constant2018variational}. On this integrative view,
projection and coupling are not rival theories of what imagination
\emph{is}, but name the two ends of a single, measurable dimension:
how heavily the ecological factors of attraction weight the attractors
at which a tradition's mythic forms come to rest.
...
% The roadmap sits at the end of the Introduction, where it is first
% cited, so that it remains Figure 1 now that Methods precedes Results.
\begin{figure}[h!t]
\centering
...
figure pointers index the Results subsections.}
...
Data~\citep{inaturalist2024} across the 14 WWF biomes. An image-side
sanity filter drops obvious non-nature content by SigLIP-2 zero-shot
scoring against twelve negative prompts covering people, indoor
scenes, vehicles, documents and museum specimens (verbatim prompts in
Supplementary Section~S7): an image is dropped if its maximum cosine
similarity to any negative exceeds $0.08$. The threshold is lenient,
removing only obvious cases, so that species-in-habitat close-ups and
natural outdoor scenes are retained, which is the content iNaturalist
is biased toward by design.

For the curated landscape comparison, the Places365
component~\citep{zhou2018places} is drawn from scene categories whose
semantic interpretation unambiguously corresponds to a single WWF
biome (e.g.\ \emph{tundra} $\to$ arctic tundra; \emph{rainforest}
$\to$ tropical moist broadleaf; \emph{forest/broadleaf} $\to$
temperate or tropical dry broadleaf; \emph{snowfield/mountain\_snowy}
$\to$ boreal/taiga; \emph{lagoon/swamp/creek/coast} $\to$ mangroves);
categories that mix biomes were excluded. Each candidate is then
passed through a strict-landscape SigLIP-2 filter built from five
positive and eleven negative prompts (Supplementary Section~S7), which
drops anything that is not a wide-shot natural scene. The
strict-landscape score is $\mathrm{mean}(\text{positives}) -
\max(\text{negatives})$; an image is dropped if its
$\max(\text{negatives}) > 0.18$, and the remaining top-$K$ images per
biome are kept, yielding $1{,}655$ scene photographs across the $11$
biomes Places365 covers. This filter is intentionally stricter than
the iNaturalist one, in particular removing single-species close-ups,
because the Places365 panel functions as a scene-only robustness check
on biome-character imagery, complementary to iNaturalist's
...
sentences (NLTK Punkt sentence tokenizer, with a terminal-punctuation
...
tokens highlighted in red: the species name ``медвежий''
(\emph{bear}), the cultural anchor ``шамана'' (\emph{shaman}), and
the ethnonym ``Маккензи'' (\emph{Mackenzie}). \emph{Stage 1 (Class word + placeholder).} An
...
tradition is assigned to one of six Glottolog macro-areas by its
geographic centroid; under each of $1{,}000$ permutations every
...
(Figure~\ref{fig:fig-recovery}C--D).

\subsection*{Use of AI tools}
Two classes of AI tool were used to analyse data in this study, and both
are described in full above; we consolidate the declaration here.
\emph{(1)~Text anonymisation.} The identity vocabulary was removed from
the Berezkin abstracts by \texttt{gemini-3.5-flash} (Google), accessed
via the Gemini API on 1 June 2026~\citep{googlegemini2026}, in the
two-pass procedure and under the verbatim prompt rules set out in
\emph{Controls against biome cues}. The pipeline is deterministic given
its inputs and prompts, both of which are released with the code; the
resulting LLM-clean corpus is released rather than regenerated, so the
analyses can be reproduced without API access. Its residual error rate
is quantified by the held-out $200$-motif audit reported above rather
than assumed. \emph{(2)~Embedding.} Motif text and imagery were encoded
with sentence-pooled SigLIP-2-large, and replicated across M-CLIP,
OpenCLIP-LAION-2B and OpenCLIP-OpenAI; all are open-weight models
obtained from public checkpoints. Source text is Berezkin's catalogue,
cited throughout. No AI tool is credited as an author.
% TODO(authors): EHS also requires declaring AI tools used to *generate
% text in the manuscript* (as you would a language-editing service),
% in the Acknowledgements. Decide what to state here and add it there.

% =============================================================================
% EHS requires these five statements, in this order, at the end of the
% manuscript and before the References section.
% =============================================================================
...
biomes (Supplementary Figure~\ref{fig:figS-corpus}). After the full
anonymisation
...
accuracy $0.68$ versus $0.50$; Figure~\ref{fig:fig-recovery}C--D).
...
% EHS caps display items at 8. The iNaturalist and Places365 recovery
% panels are merged into a single figure so the scene-corpus replication
% stays in the main text rather than being cut for a slot.
\begin{figure}[h!t]
\centering
\includegraphics[width=\textwidth]{fig_biome_recovery.png}\\[0.6em]
...
labels, on both image corpora.} Each motif is represented by its
residualised cosine-similarity profile across an image corpus; biome
labels are never used to build the affinity geometry, and ``own biome''
is the biome contributing the most of a motif's traditions.
\emph{Top row, iNaturalist} ($46{,}481$ species photographs).
\textbf{(A)}~Own-biome retrieval: the nine well-sampled biomes are
ranked by each motif's taxon-stratified mean similarity to their
images; the curve gives the fraction of motifs whose own biome falls
within the top $k$ ranks (orange) against the chance diagonal (grey).
Own biome reaches the $64$th percentile on average ($p<.001$,
$2{,}000$ permutations). \textbf{(B)}~Per-biome decodability: balanced
accuracy of a five-fold one-versus-rest logistic probe on the PCA-50
affinity vectors (bars) against a $200$-shuffle label-permuted null
(mean $\pm 2$ sd); all nine biomes decode above chance at
Benjamini--Hochberg FDR $q<.05$, at a mean of $0.66$ versus $0.50$.
\emph{Bottom row, Places365} ($1{,}655$ landscape scenes). Because
these scenes carry no iconic-taxon labels, own-biome retrieval uses
the marginal per-biome similarity rather than the taxon-stratified
one. \textbf{(C)}~Over the seven biomes with sufficient scene and
motif support, own biome reaches the $61$st percentile on average
($p=0.0005$, $2{,}000$ permutations). \textbf{(D)}~All seven biomes
decode above their $200$-shuffle null at FDR $q<.05$, at a mean
balanced accuracy of $0.68$ versus $0.50$.}
...
above their label-shuffled null (Figure~\ref{fig:fig-recovery}A--B),
and this recovery replicates on the Places365 scene corpus
(Figure~\ref{fig:fig-recovery}C--D). Removing the names, holding
...
This project uses Yu.~E. Berezkin's \emph{Analytical Catalogue of World
Mythology and Folklore}; we thank him for making the catalogue and its
geographic coding publicly available. We also acknowledge the WWF
Terrestrial Ecoregions of the World, iNaturalist Open Data, Places365,
and the YFCC100M dataset.
% TODO(authors): add any individuals or organisations that gave advice or
% non-financial support. Financial support belongs in the next section.

\section*{Author Contributions}
% TODO(authors): CONFIRM BEFORE SUBMISSION -- this is a draft, not a record.
% EHS wants a short statement of how each author contributed.
ABP conceived and designed the study, built the data pipeline, performed
the analyses, and wrote the article. ME contributed to the interpretation
of the results and to revising the article. Both authors approved the
final manuscript.

\section*{Financial Support}
% TODO(authors): CHOOSE ONE. If any grant supported this work, replace the
% sentence below with e.g. "This work was supported by the Fonds de
% recherche du Quebec (A.B.P., grant number XXXX)."
% The wording below is EHS's prescribed text for unfunded research.
This research received no specific grant from any funding agency,
commercial or not-for-profit sectors.

\section*{Conflicts of Interest}
% TODO(authors): confirm this is accurate for both authors.
Antoine Bellemare-Pepin and Mar Estarellas declare none.

\section*{Research Transparency and Reproducibility}
All code, the sentence-pooled SigLIP-2 embeddings, the LLM-clean motif
texts, the held-out anonymisation audit (the per-criterion coding of the
200 audited motifs, released as
\texttt{anonymisation\_audit\_200motif.csv}), and the earlier rule-based
hypernym maps retained for completeness are available at
\url{https://github.com/AntoineBellemare/pareidolia-in-nature}. The
Berezkin catalogue is accessible at
\url{http://www.ruthenia.ru/folklore/berezkin/}. The WWF Terrestrial
Ecoregions shapefile, the iNaturalist Open Data photographs, and the
Places365 scenes are redistributed by their originators under their own
terms and are not mirrored here; the manifests needed to reconstruct
the exact image sets are included in the repository.
...
\subsection*{S0. Corpus coverage}

Figure~\ref{fig:figS-corpus} maps the sample against the biome
geography it is drawn from. It is descriptive: no claim in the main
text rests on it, which is why it sits here rather than among the
eight main display items.
...
scenes alike; Figure~\ref{fig:fig-recovery}.)
...
% =============================================================================
\subsection*{S7. Image-filter prompts}\label{sec:filter-prompts}

The two image corpora are filtered by SigLIP-2 zero-shot scoring
against the prompt sets below (\emph{Image corpora}). They are
reproduced verbatim so the filters can be reimplemented exactly.

\paragraph{iNaturalist sanity filter.} An image is dropped if its
maximum cosine similarity to any of these twelve negative prompts
exceeds $0.08$:
...
laboratory''}, \emph{``a city skyline or street''}, \emph{``a car or
vehicle''}, \emph{``a road sign''}, \emph{``a black or all-white
image''}, \emph{``a museum specimen on a white background''},
\emph{``a screenshot or document''}.

\paragraph{Places365 strict-landscape filter.} The score is
$\mathrm{mean}(\text{positives}) - \max(\text{negatives})$, and an
image is dropped if $\max(\text{negatives}) > 0.18$.

\emph{Five positive prompts:}
\emph{``a wide-shot natural landscape with no people''},
\emph{``a panoramic photograph of natural scenery''},
\emph{``an empty natural habitat photographed from a distance''},
\emph{``a landscape photo of natural terrain showing vegetation and
horizon''}, \emph{``a wild ecosystem with no human-made structures''}.

\emph{Eleven negative prompts:}
\emph{``a person in the photograph''}, \emph{``a portrait of one or
more humans''}, \emph{``a close-up photo of a single animal''},
\emph{``a single bird filling the frame''}, \emph{``a car or
vehicle''}, \emph{``a boat''}, \emph{``a road or street''}, \emph{``a
man-made structure or building''}, \emph{``an indoor scene''},
\emph{``a person riding a horse or animal''}, \emph{``a tourist
