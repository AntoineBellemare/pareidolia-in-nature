...
photographs ($n=47{,}478$ images), with $\Delta$ stratified within
...
strict-landscape scenes ($n=1{,}675$ images, 11 biomes); because
...
grey asterisks denote nominal $p<.05$. The two corpora agree on
...
Places365 strict-landscape comparison set, $1{,}675$ scene
...
artefact of one corpus's curatorial peculiarities. Projected onto
...
across-biomes mean $\Delta$ on iNaturalist, both marginal and
within-iconic-taxon stratified, partitioned by motif breadth into
three groups (Spec~A: motifs touching $\leq 3$ biomes;
semi-universal: 4--7 biomes; universal: $\geq 8$ biomes). Both
definitions show monotonic decay: stratified $\mu\Delta$ drops
from $+0.58\times 10^{-3}$ (Spec~A) through $+0.44\times 10^{-3}$
(semi-universal) to $+0.12\times 10^{-3}$ (universal), the
pattern most naturally expected if local ecological affordances
contribute to motif stabilisation. \emph{Panel B}: per-biome stratified $\Delta$ within
...
recoverable three ways without ever using biome labels to construct
...
($p<.001$, $2{,}000$ label permutations) and is the single
most-similar of the nine biomes $13.1\%$ of the time against an
$11.1\%$ chance rate (Figure~\ref{fig:fig-recovery}A). Second, the
taxon-stratified mean similarity of each biome's motifs to each
biome's images, $z$-scored within row, places the own-biome cell
above its row average for six of the nine biomes
(Figure~\ref{fig:fig-recovery}B), the same Tundra, Desert, Boreal
and grassland diagonal that drives the headline map. Third, a
...
(Figure~\ref{fig:fig-recovery}C). The same ordering recurs
throughout. The biome signal is real and recoverable from the
model's image affinities alone, but it is a thin overlay on a
representation that is mostly about the plants and animals the
myths name and the scenes they describe.
...
\textbf{(B)}~Biome$\,\times\,$biome alignment. Taxon-stratified mean
similarity of each row-biome's motifs to each column-biome's
images, $z$-scored within row; the boxed diagonal is above its row
average in six of nine biomes. \textbf{(C)}~Per-biome decodability.
Balanced accuracy of a five-fold one-versus-rest logistic probe on
the PCA-50 affinity vectors (bars) against a label-shuffled null
(ticks); every biome decodes above chance. Throughout, ``own
biome'' is the biome contributing the most of a motif's traditions,
and the affinity vectors are built without reference to it.}
...
manufacture a positive on biome-neutral content; it persists in
motifs whose anonymised text alone provides minimal text-only
biome predictability, so it is not reducible to a one-sided
lexical artefact. What is doing the work, in other words, is the
...
this point by \emph{removing} naming from the text. A
complementary analysis on the original, un-anonymised myths
reaches the same conclusion by instead holding the naming
constant statistically (Supplementary
Section~\ref{sec:ladder}). Keeping the raw Russian myths intact,
we measure three identity-naming channels separately --- the
species named in the text, the geographic regions, and the
ethnolinguistic groups --- and test whether the full-myth
alignment survives matched-permutation nulls that shuffle biome
only among motifs with the same naming content. Each channel is
biome-diagnostic on its own (a bag of species names is in fact
\emph{more} biome-diagnostic than the full myth), and naming
accounts for roughly $40$ to $70\%$ of the alignment depending on
the channel. But $11$ of $14$ biomes retain a significant
alignment under a joint null that holds species, place, and
ethnonym constant simultaneously, and an independent
embedding-space projection of the species axis attenuates the
effect from $+0.47$ to $+0.29$--$0.35\times 10^{-3}$ while leaving
it significant in $11$--$13$ biomes. The two strategies,
removing the names and holding the names constant, converge: the
biome--mythology alignment is not reducible to the trivial
channel by which a biome's mythology names that biome's species,
places, and peoples.
...
The iNaturalist component consists of $47{,}478$ images drawn from
...
(mean $23$ sentences per motif; $50{,}032$ unique sentences across
...
to build it. We report three diagnostics over the nine biomes with
...
reassign each motif's own biome at random. Second, the
biome$\times$biome matrix is the same taxon-stratified similarity
averaged over each row-biome's motifs and $z$-scored within row.
Third, per-biome decodability is the balanced accuracy of a
five-fold one-versus-rest $\ell_2$-regularised logistic probe on
the PCA-reduced vectors, compared against the same probe trained on
shuffled labels. The unsupervised UMAP layout and its
...
and the earlier rule-based hypernym maps released for completeness
are available at [repository URL placeholder]. The Berezkin
...
angles: (i) the headline robustness atlas summarised in the main
Results, with the four panels shown at full size; (ii) a
...
\subsection*{S6. Identity-naming decomposition on the original
myths}\label{sec:ladder}

The anonymisation pipeline addresses the concern that the alignment
reflects only biome-correlated naming by \emph{removing} species,
place, and biome vocabulary from the text. A complementary strategy
is to \emph{keep} the original myths intact and instead control for
the naming channels statistically. This supplementary analysis does
that, on the raw (un-anonymised) Russian Berezkin abstracts, and
decomposes the alignment into three separately-measured
identity-naming components.

We extract three identity classes per motif and keep them separate.
\emph{Species}: every specific named organism (fauna and flora) in
the myth, extracted by an LLM reading the Russian text; an
independent adversarial audit of 120 motifs estimates $96.7\%$
recall and $95.3\%$ precision against the source text. \emph{Place}:
the WWF-codable geographic regions associated with each motif in
Berezkin's metadata. \emph{Ethnonym}: the ethnolinguistic groups
(tradition names) that carry each motif in the catalogue. Each
class is rendered as a bag of its entities and embedded with the
same sentence-pooled SigLIP-2 pipeline as the full myth, in the
raw-Russian frame, and compared against the same iNaturalist
images. This is a self-contained decomposition in the
raw-Russian frame; its absolute magnitudes are not directly
comparable to the English LLM-clean headline.

On its own, each identity channel is biome-diagnostic
(Figure~\ref{fig:figS-ladder}, Panel A): the full original myth
gives residualised marginal $\mu\Delta = +0.47\times 10^{-3}$
(12/14 biomes significant), a species-only bag gives
$+0.75\times 10^{-3}$ (14/14), an ethnonym-only bag
$+0.69\times 10^{-3}$ (14/14), and a place-only bag
$+0.26\times 10^{-3}$ (9/14). A pure bag of species names is in
fact \emph{more} biome-diagnostic than the full myth, which dilutes
the species cue with narrative content. Species naming is therefore
a genuinely strong channel, exactly the concern.

We test whether the full-myth alignment survives controlling for
each channel with a matched-permutation null
(Figure~\ref{fig:figS-ladder}, Panel B): biome labels are shuffled
only among motifs that are nearest neighbours in the relevant
identity-bag embedding space ($60$ K-means blocks), so the identity
content is held approximately constant while biome assignment is
broken. Under the species-matched null, the null reproduces $38\%$
of the observed $\Delta_{\text{full}}$ but $11$ of $14$ biomes
retain observed $\Delta$ above the null at $p<.05$. Under the
ethnonym-matched null, $10/14$ survive; under the place-matched
null, $7/14$ (the place null is near-circular, since each biome is
\emph{derived} from the WWF region assignment, so holding place
constant nearly holds biome constant). Under a joint null that
blocks on all three identity classes simultaneously, $11/14$ biomes
still survive. An independent embedding-space test
(Figure~\ref{fig:figS-ladder}, Panel C) projects the top species
directions out of the full-myth embeddings and recomputes
$\Delta$: $\mu\Delta$ falls from $+0.47$ to $+0.29$--$0.35\times
10^{-3}$ but remains significant in $11$--$13$ of $14$ biomes
across $5$, $10$, and $20$ removed directions.

The two independent methods converge. Identity naming (species,
place, and peoples) accounts for a substantial fraction of the
biome--mythology alignment on the original myths, roughly $40$ to
$70\%$ depending on the channel, but the majority of biomes retain
a significant alignment after that naming is held constant. The
effect measured on the un-anonymised myths is therefore not
reducible to the trivial channel by which biome-$b$ mythology names
biome-$b$ species, places, and peoples --- consistent with, and
independent of, the anonymisation-based evidence in the main text.

\begin{figure}[h!t]
\centering
\includegraphics[width=\textwidth]{figS_ladder_decomposition.png}
\caption{\textbf{Identity-naming decomposition on the original
(un-anonymised) Russian myths.} \emph{Panel A}: per-biome
residualised marginal $\Delta$ for the full original myth and for
three separately-measured identity baselines (species, place,
ethnonym), raw-Russian frame, iNaturalist images. \emph{Panel B}:
matched-permutation-null ladder. For each identity class, and for
all three jointly, biome labels are shuffled only among motifs
matched on that class's identity-bag embedding (60 K-means blocks);
bars show observed $\mu\Delta_{\text{full}}$ (black) versus the
matched-null mean (gold), with the count of biomes whose observed
$\Delta$ exceeds the null at $p<.05$. The place null is
near-circular because biome is derived from the region metadata.
\emph{Panel C}: independent embedding-space control --- the top
$k$ species-subspace directions are projected out of the full-myth
embeddings and $\Delta$ is recomputed ($k = 5, 10, 20$); $\mu\Delta$
attenuates but stays positive and significant.}
\label{fig:figS-ladder}
\end{figure}
