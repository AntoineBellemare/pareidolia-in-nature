...
assignment (each biome drawn from an independent random stream and
the stratified statistic recomputed under each permutation), with
permutation $p = (1 + \#\{\text{null} \geq \text{obs}\}) / (1 + 1000)$
and FDR control across biomes via the Benjamini--Hochberg
procedure~\citep{benjamini1995controlling}; the same $+1$ smoothing
and independent per-test seeding are used by the matched-permutation
and unsupervised-recovery nulls below.
...
percentile of this swap distribution (with Benjamini--Hochberg FDR
across biomes). The null is defined once here and applied in the
...
two anonymisation strategies---removing the names and holding them
...
motifs (LLM-strip $\mu\Delta^{\text{strat}} = +0.58 \rightarrow
+0.44 \rightarrow +0.12\times 10^{-3}$; raw matched null $+0.94
\rightarrow +0.42 \rightarrow +0.26$). And the per-taxon structure
(Figure~\ref{fig:figS-converge-taxon}) lines up cell for cell: the
per-(biome $\times$ taxon) $\Delta$ matrices correlate at
$r = +0.64$, with the same vertebrate-and-plant taxa carrying the
signal under both controls. We read this convergence as evidence
