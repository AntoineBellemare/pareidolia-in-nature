...
original myth gives $+0.42\times 10^{-3}$ ($10/14$), strikingly
close to the $+0.41\times 10^{-3}$ the \emph{anonymised} corpus
reaches on the same statistic --- two opposite roads, removing the
names and keeping them, arriving at the same number. A bag of
...
simultaneously --- the same count that survives the Glottolog
cultural-geographic swap, two stringent and unrelated controls
leaving the same handful of biomes standing. (The place channel
alone is near-circular, since biome is derived from the region
metadata, so we read the joint and species-only nulls rather than
place.) An independent embedding-space test
...
against iNaturalist images. \emph{Panel A}: per-biome decomposition
of the full original myth and three separately-measured identity
baselines (species, place, ethnonym bags). A species-only bag is
more biome-diagnostic than the full myth, and this survives
stratification. \emph{Panel B}: matched-permutation-null ladder ---
for each identity class, and for all three jointly, biome labels
are shuffled only among motifs matched on that class's
identity-bag embedding ($60$ K-means blocks); bars give observed
$\mu\Delta_{\text{full}}$ (black) versus the matched-null mean
(gold), annotated with the count of biomes whose observed $\Delta$
exceeds the null at $p<.05$. The place null is near-circular
because biome is derived from the region metadata. \emph{Panel C}:
independent embedding-space control --- the top $k$
...
blocking. The species-subspace projection removes the top $k$
principal directions of the species-bag embeddings from the
full-myth embeddings ($k = 5, 10, 20$) and recomputes the
stratified $\Delta$. The extraction prompts, audit, and per-biome
tables are released with the code.
